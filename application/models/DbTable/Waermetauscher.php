    ...
    public function updateWaermetauscher(Application_Model_Waermetauscher $waermetauscher) {
    	try{
	    	...
	    	if(key_exists('waermetauscherUnterkategorie', $waermetauscherData)) {
	    		//Unterkategorien zum löschen laden
	    		$wtU2delete = $waermetauscher->getWaermetauscherUnterkategorie2delete();
	    		if(!empty($wtU2delete)){
	    			foreach($wtU2delete as $value) {
	    				$this->getWaermetauscherUnterkategorieDbTable()->deleteWaermetauscherUnterkategorie($value);
	    			}
	    		}
